...
var FrontmatterSchema = external_exports.object({
  ...
  slug: external_exports.string().min(1, "Slug cannot be empty").optional(),
  canonical: external_exports.string().url("Canonical URL must be a valid URL").optional(),
  tags: external_exports.array(external_exports.string()).optional().default([]),
  published: external_exports.boolean().optional().default(false),
...
var BlogPost = class {
  ...
  get slug() {
    ...
    return this.frontmatter.title.toLowerCase().trim().replace(/[^a-z0-9\s-]/g, "").replace(/\s+/g, "-");
  }
  get isPublished() {
    return this.frontmatter.published ?? false;
  }
  calculateChecksum(str2) {
    let hash = 0;
...
var SourceConfigSchema = external_exports.object({
  folder: external_exports.string().default("content/blog"),
  match: external_exports.string().default("**/*.md"),
  files: external_exports.array(external_exports.string()).optional()
});
var TrackingConfigSchema = external_exports.object({
  provider: external_exports.enum(["file", "memory"]).default("file"),
  file: external_exports.string().default(".devpublisher/state.json")
});
var PipelineConfigSchema = external_exports.object({
  validators: external_exports.array(external_exports.string()).default(["frontmatter"]),
  transformers: external_exports.array(external_exports.string()).default(["canonical-url"]),
  tracking: TrackingConfigSchema.default({})
});
var DevPublisherConfigSchema = external_exports.object({
...
var FileTrackingProvider = class {
  ...
  saveState() {
    try {
      ...
      fs2.writeFileSync(this.filePath, JSON.stringify(this.state, null, 2), "utf-8");
    } catch (err) {
  ...
  async setPlatformState(slug, checksum, platformId, platformState) {
...
var MemoryTrackingProvider = class {
...
var RegistryManager = class {
  ...
  publishers = new PublisherRegistry();
  reporters = new ReporterRegistry();
  register(plugin) {
    switch (plugin.type) {
      case "loader":
        this.loaders.register(plugin);
        break;
      case "validator":
        this.validators.register(plugin);
        break;
      case "transformer":
        this.transformers.register(plugin);
        break;
      case "publisher":
        this.publishers.register(plugin);
        break;
      case "reporter":
        this.reporters.register(plugin);
        break;
      default:
...
var FileLoaderPlugin = class {
  ...
  version = "1.0.0";
  type = "loader";
  description = "Loads Markdown files from local file system";
  async load(config) {
    const posts = [];
    ...
    if (config.files && config.files.length > 0) {
      ...
      const folderPath = path3.resolve(process.cwd(), config.folder);
      if (fs3.existsSync(folderPath)) {
        const entries = fs3.readdirSync(folderPath, { recursive: true, withFileTypes: true });
        for (const entry of entries) {
          if (entry.isFile() && entry.name.endsWith(".md")) {
            const fullPath = path3.join(entry.parentPath || entry.path || folderPath, entry.name);
...
var FrontmatterValidatorPlugin = class {
  ...
  version = "1.0.0";
  type = "validator";
  description = "Validates article frontmatter against domain schema";
  async validate(post) {
...
var CanonicalUrlTransformerPlugin = class {
  ...
  version = "1.0.0";
  type = "transformer";
  description = "Ensures canonical URL is formatted and present";
  async transform(post, _platformId) {
    return post;
  }
};
...
var ConsoleReporterPlugin = class {
  ...
  version = "1.0.0";
  type = "reporter";
  description = "Formats and outputs execution summary to console";
  async report(result) {
    console.log("\n==================================================");
    ...
    for (const articleRes of result.results) {
      console.log(`
\u{1F4C4} Article: ${articleRes.title} (${articleRes.slug})`);
...
var ExecutionContext = class {
  ...
  metadata = {};
  constructor(options2) {
    this.id = options2.runId ?? `run-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`;
    this.config = options2.config;
    this.logger = options2.logger ?? new ConsoleLogger();
    this.cache = options2.cache ?? new MemoryCacheProvider();
    this.tracking = options2.tracking ?? new MemoryTrackingProvider();
          ...
          if (result.status === "published" || result.status === "updated") {
            if (result.externalId && result.url) {
              await ctx.tracking.setPlatformState(post.slug, post.checksum, platformId, {
                ...
                url: result.url,
                publishedAt: platformState?.publishedAt || (/* @__PURE__ */ new Date()).toISOString(),
  ...
  static load(configPath, overrides) {
    ...
    if (fs4.existsSync(absolutePath)) {
      ...
      if (absolutePath.endsWith(".json")) {
        ...
        rawConfig = _ConfigLoader.parseSimpleYaml(content);
      }
    }
    const merged = {
      ...rawConfig,
  ...
  static parseSimpleYaml(content) {
    ...
    for (const line of lines) {
      ...
      if (indent === 0 && line.includes(":")) {
        ...
        const key = parts[0].trim();
        const val = parts.slice(1).join(":").trim();
        result[currentSection][currentSubSection][key] = _ConfigLoader.parseYamlValue(val);
      }
    }
    return result;
  }
...
var DevPublisherEngine = class {
  ...
  constructor(options2 = {}) {
    this.config = ConfigLoader.load(options2.configPath, options2.configOverrides);
  ...
  async run(contextOverrides) {
    const trackingProvider = contextOverrides?.tracking || (this.config.pipeline.tracking.provider === "file" ? new FileTrackingProvider(this.config.pipeline.tracking.file) : void 0);
  ...
  async validate(contextOverrides) {
    const ctx = new ExecutionContext({
      config: this.config,
      ...contextOverrides
    });
...
var DevtoPublisher = class {
  ...
  async publish(post, options2) {
    const apiKey = options2?.apiKey || this.config?.apiKey || process.env.DEVTO_API_KEY;
    if (!apiKey) {
      return {
...
var DevtoPublisherPlugin = class {
  ...
  version = "1.0.0";
  type = "publisher";
  description = "Official Dev.to cross-posting publisher plugin";
...
function createEngine(target, options2) {
  ...
  if (options2?.platforms) {
    const platformList = options2.platforms.split(",").map((p) => p.trim());
...
program2.command("validate").argument("[target]", "Markdown file or directory to validate", "content/blog").option("-c, --config <config>", "Path to devpublisher.yml config file").description("Validate frontmatter and syntax of markdown articles").action(async (target, options2) => {
  try {
    const engine = createEngine(target, options2);
...
program2.command("list").description("List all registered plugins and publisher platforms").action(() => {
